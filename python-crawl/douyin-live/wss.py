...
logging.basicConfig(level=logging.INFO,
                    format='%(message)s',
                    filename='logs/live_messages.log',  # 指定日志文件的路径
                    filemode='w',  # 'a' 表示追加模式，如果是 'w' 则每次运行都会覆盖文件内容
                    encoding='utf-8')  # 确保以 UTF-8 编码写入，支持中文字符
    ...
    def parse_gift_msg(self, msg_payload):
        ...
        try:
            ...
            message = {
                ...
                "combo_count": gift_msg.comboCount
            }
            logging.info(json.dumps(message, ensure_ascii=False))
        except Exception as e:
...
if __name__ == '__main__':
    room = Room("https://live.douyin.com/489347072218")
    room.connect()
